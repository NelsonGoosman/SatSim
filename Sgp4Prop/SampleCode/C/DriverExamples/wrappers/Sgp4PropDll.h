...
typedef int (STDCALL *fnPtrSgp4Init)(__int64 apAddr);
...
typedef int (STDCALL *fnPtrSgp4LoadFileAll)(char sgp4InputFile[512]);
...
typedef int (STDCALL *fnPtrSgp4SaveFile)(char sgp4File[512], int saveMode, int saveForm);
...
typedef int (STDCALL *fnPtrSgp4PosVelToKep)(int yr, double day, double pos[3], double vel[3], double posNew[3], double velNew[3], double xa_kep[6]);
...
// returns 0 if the reepoch is successful, non-0 if there is an error.
typedef int (STDCALL *fnPtrXpGetNativeElts)(__int64 satKey, double ds50UTC, double xa_eqnx[6], double xa_eqnx_dot[6]);
...
// xa_eqnx_dot        Equinoctial Element rates (subtract earth rotation rate from Longitude Rate) at reEpoch time (out-Double[6])
// returns 0 if the propagation is successful, non-0 if there is an error.
typedef int (STDCALL *fnPtrXpReepochGetNativeElts)(__int64 satKey, double reEpochDs50UTC, char csvLine[512], double xa_eqnx[6], double xa_eqnx_dot[6]);
  // GP types (TLE ephemeris types) - They are different than ELTTYPE
  static const int  
     GPTYPE_SGP4 =  0,    // TLE SGP4 gpType (Brouwer mean motion)
     GPTYPE_PPT3 =  3,    // TLE PPT3 gpType (Brouwer mean motion)
     GPTYPE_XP   =  4,    // TLE SGP4-XP gpType (Brouwer mean motion)
     GPTYPE_NONE = 99;    // Not a GP type 
  ...
  static const int  
     ...
     GP_ERR_INCOUTOFBND = 8,    // Inclination out of bounds
     GP_ERR_MNMOTNEG    = 9,    // Mean motion is negative 
     GP_ERR_DECAY       =10,    // Satellite decayed
     GP_ERR_UNKNOWNTYPE =99;    // Unknown GP element type
...
extern fnPtrSgp4LoadFileAll                Sgp4LoadFileAll;
extern fnPtrSgp4SaveFile                   Sgp4SaveFile;
...
extern fnPtrXpGetNativeElts                XpGetNativeElts;
extern fnPtrXpReepochGetNativeElts         XpReepochGetNativeElts;
