...
// returns Current elset key mode (see ELSET_KEYMODE_?)
...
// returns Current elset key mode (see ALL_KEYMODE_?)
...
// returns 0 if the set is successful, non-0 if there is an error.
typedef int (STDCALL *fnPtrSetDupKeyMode)(int dupKeyMode);


// Gets current DUPLICATION key mode
// returns Current duplication key mode (see DUPKEY_?)
typedef int (STDCALL *fnPtrGetDupKeyMode)();
...
typedef void (STDCALL *fnPtrGetErrMsg)(int errCode, char errMsg[128]);
...
// returns 0 if the set is successful, non-0 if there is an error.
typedef int (STDCALL *fnPtrSetErrMsgMode)(int mode);


// Gets error message mode 
// mode = 0: Only last error message is stored and then retrieved via GetLastErrMsg(). Appropriate for single-thread applications
// mode = 1: Each thread stores and retrieves its own error message via GetErrMsg(). Appropriate for multi-thread applications
// returns Error message mode: 0= use GetLastErrMsg(), 1= use GetErrMsg() to retrieve appropriate error message
typedef int (STDCALL *fnPtrGetErrMsgMode)();
  ...
  static const int  
     ...
     ELTTYPE_TLE_PPT3  = 8;    // Element type - PPT3 Tle type 3 - XP
  ...
  static const int  
     ...
     BADSATNUM = -1,      // Bad satellite number
...
extern fnPtrSetDupKeyMode                  SetDupKeyMode;
extern fnPtrGetDupKeyMode                  GetDupKeyMode;
extern fnPtrGetErrMsg                      GetErrMsg;
extern fnPtrSetErrMsgMode                  SetErrMsgMode;
extern fnPtrGetErrMsgMode                  GetErrMsgMode;
