...
void* LoadLib(const char* dllName)
{
   ...
   if (hLib == NULL)
   {
      ...
      #ifdef _WIN32
         printf("GetLastError() returns code # %lu\n", GetLastError());
      #else
         printf("dlerror() returns message %s\n", dlerror());
      #endif
...
void* GetFnPtr(void* hLib, char* fName)
{
   ...
   if (fnctPtr == NULL) 
   {
      ...
      //exit(1); //Allow newer driver code to still work with older libraries
   }
